 ...
 * -模拟生成日志数据
 * -交给Nginx进行反向代理
 * -交给日志处理服务器 将日志发送到kafka的ods_base_log
 * -BaseLogApp从ods读取数据，进行分流，将分流的数据发送到kafka的dwd(dwd_page_log)
...
//        userJumpJsonStrDS.print("userJump>>>>");
        ...
        //3.2 转换uv流
        SingleOutputStreamOperator<VisitorStats> uvStatsDS = uvJsonStrDS.map(new MapFunction<String, VisitorStats>() {
            @Override
            public VisitorStats map(String jsonStr) {
                //将json格式字符串转换为json对象
                JSONObject jsonObj = JSON.parseObject(jsonStr);
        ...
        //3.3 转换sv流(session_count) 其实还是从dwd_page_log中获取
        SingleOutputStreamOperator<VisitorStats> svStateDS = pvJsonStrDS.process(new ProcessFunction<String, VisitorStats>() {
            @Override
            public void processElement(String jsonStr, ProcessFunction<String, VisitorStats>.Context ctx, Collector<VisitorStats> out) throws Exception {
                //将json格式字符串转换为json对象
                JSONObject jsonObj = JSON.parseObject(jsonStr);
                //获取当前页面的lastPageId
        ...
        DataStream<VisitorStats> unionDS = pvStatsDS.union(uvStatsDS, svStateDS, userJumpStatsDS);
        ...
        //TODO 6.分组 按照地区、渠道、版本、新老访客维度进行分组,因为我们这里有4个维度，所以将他们封装称为一个Tuple4
        ...
        //TODO 8.对窗口中的数据进行聚合  聚合结束之后，需要补充统计的起止时间
